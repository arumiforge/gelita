...
* **Kenali wilayah (Tahap 3).** Setiap `[data-kenal]` (overlay di luar `<main>`) diberi `initNarrator()` mode `external`, `data-hash="0"`, `data-keyboard="0"`. Klik `[data-kenal-open]` (pin & kartu): `Sfx.unlock()` di dalam ketukan, narasi peta dihentikan, overlay `.is-open`, HUD/`<main>`/nav/skip-link `inert`, kembali ke slide 1 tanpa event, lalu `start({ userInitiated: true })`; fokus ke tombol ✕. Selama terbuka: Esc menutup, →/Spasi `advance()`, ← `prev()`. Tutup (`[data-kenal-close]`, ✕, Esc) → `stop()`, `flush()`, fokus kembali ke tombol pembuka. `onSlide` di slide terakhir menghapus `[data-kenal-badge]` wilayah itu; `onFinish` (▶ di slide terakhir) mengklik tombol akhirnya — "Masuk ke {wilayah}" memutar tirai wilayah lewat `initCurtains()`. Kembali lewat bfcache menutup overlay. Hash `#kenal-{code}` atau `#kenal-{code}-{n}` yang sudah ada saat modul siap (tombol diketuk sebelum JavaScript peta siap sehingga tautannya berjalan biasa, atau tautan langsung) membuka overlay itu di slide n, lalu hash dibersihkan dengan `history.replaceState` agar `:target` tidak tertinggal. Karena bukan ketukan di halaman ini, `Sfx.unlock()` tidak dipanggil dan `start({ userInitiated: Sfx.isUnlocked() })`: tanpa suara, teks + tombol ▶. `initRegionIntros()` mengembalikan `() => boolean` (overlay terbuka?), dan narasi peta tidak dimulai setelah tirai peta bila overlay sedang terbuka.
...
* **Lanjutan: uji audio & bahasa Inggris.** Audio WAV sementara diunggah lewat alur admin (unggah → Setujui → editor dialog `region_intro`; `level_open`/`level_done`/`ending` lewat SQL), Chromium dengan `--autoplay-policy=user-gesture-required`, 844×390 dan 1440×900. Peta tanpa ketukan tidak berbunyi. Kenali: `play` di slide 1, maju otomatis ±1,2 detik setelah `ended`, ⏸ menahan maju otomatis, ↻ memulai ulang, sakelar Otomatis mati berarti tidak maju sendiri, Esc menghentikan suara, dibuka lagi mulai slide 1. Dialog, tuntas, dan penutup: kartu ketuk → `autoplay` berantai, baris/slide tanpa audio berhenti (teks saja, ⏸/↻ tersembunyi), slide terakhir tidak berpindah halaman sendiri. EN memakai audio EN; slide tanpa audio EN tampil teks saja (audio ID tidak dipakai). Suara dimatikan berarti tanpa audio. `audio_usage_events` mencatat `play`/`autoplay`/`pause`/`replay`/`complete`. Temuan: ketukan Kenali tepat sebelum `game/map.js` siap (±0,8 detik setelah muat di 844×390) hanya mengubah hash dan overlay tidak terbuka; diperbaiki dengan penanganan hash di atas. Tampilan EN (peta, overlay, tirai wilayah & tantangan termasuk `urutkan_informasi`, kartu bab, tuntas, penutup) tidak meluber; label "Get to know {wilayah}" terlipat dua baris di kartu 844×390 dan tetap di dalam kartu.
